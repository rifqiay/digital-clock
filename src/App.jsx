...
function App() {
  ...
  useEffect(() => {
    const getAllKeysFromLocalStorage = () => {
      const keys = Object.keys(localStorage).filter((key) => key.startsWith('task'));
      return keys;
    };
...
const durationFormatted = `${hours.toString().padStart(2, '0')}Jam ${minutes.toString().padStart(2, '0')}Menit`;
  ...
  return (
    <div className="bg-[#10172A] h-screen text-[#E2E8F0] flex justify-center items-center">
      <p className="absolute right-3 top-3 cursor-pointer" onClick={()=>document.getElementById('my_modal_1').showModal()}>Todo
      </p>
      <Modal/>
      <p className="absolute right-3 bottom-3 cursor-pointer" onClick={handleClearTask}>Hapus semua tugas</p>
      <div>
        ...
        </h1>
        {
          getTask.map((item, i) => (
            <div className="flex gap-4 mt-1" key={i}>
              ...
              <span>Mulai: {dayjs(item.startAt).locale('id').format('YY-MM-DD HH:mm')}</span>
            ...
            <input type="checkbox" className="checkbox checkbox-accent" checked={item.status} onChange={() => handleFinishTask(item)} disabled={item.status} />
